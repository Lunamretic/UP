...
public class Server implements HttpHandler {
    ...
    private void doDelete(HttpExchange httpExchange) throws Exception {
        String idToDelete = messageExchange.getIdToDelete(httpExchange.getRequestBody());

        for (int i = 0; i < messageList.size(); i++) {
            if (idToDelete.equals(messageList.get(i).getId())) {
                System.out.println("Message \"" + messageList.get(i).getMessage() +
                        "\" from user \"" + messageList.get(i).getAuthor() + "\" - deleted." );
